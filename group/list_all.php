...
$userid  = trimInput(array_key_exists('userid',  $_REQUEST) ? $_REQUEST['userid'] : 0);	   
if(empty($userid)){
	echoErr('missing args');	
}


$ret = get_group_list($g_readMdb, $userid);
if($ret['ret'] != 0){	
...
function get_group_list($mdb, $userid){
	$retArr = array();
	try{
				...
				foreach ($pureGroupArr as $groupId){
					...
					if($pstmt->execute(array($groupId))){
						...
						if($resNum == 1){									
							...
							$retArr['ret'] = 16;return $retArr;//该群不存在								
						} */		
					}else{
